...
CONSTRAINT fk_from_RT FOREIGN KEY (fromAccountID) REFERENCES Accounts(accountID),
CONSTRAINT fk_to_RT FOREIGN KEY (toAccountID) REFERENCES Accounts(accountID)
);
...
DELIMITER $$
CREATE PROCEDURE MonthlyMonyTransit( IN theDate DATETIME, IN amount DECIMAL(10,2))
BEGIN
START TRANSACTION;
-- Tự động chuyển tiền từ tài khoản nguồn đến tài khoản đích theo tần suất đã định.
     CASE
         WHEN DATE(theDate) = DATE(NOW()) THEN CALL TransferMoney(9999, 8888, amount);
     END CASE;
-- Cập nhật trường NextTransactionDate trong bảng RecurringTransactions sau mỗi lần giao dịch.
     ...
     VALUES 
     (9999, 8888, amount, theDate, 'monthly', DATE_ADD(NOW(), INTERVAL 1 MONTH));
     
COMMIT;
END $$
DELIMITER ;

-- Tự động kiểm tra định kì.

DELIMITER $$
CREATE EVENT AutoMonthlyTransfer
ON SCHEDULE EVERY 1 DAY
STARTS CURRENT_TIMESTAMP()
DO
BEGIN
     CALL MonthlyMonyTransit('2025-12-05', 500000);
END $$
DELIMITER ;
...
ALTER EVENT AutoMonthlyTransfer ENABLE;

ALTER EVENT AutoMonthlyTransfer DISABLE;

DROP EVENT IF EXISTS AutoMonthlyTransfer;
